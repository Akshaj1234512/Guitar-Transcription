...
def cache_key_for(wav_path: str, cache_key_mode: str) -> str:
    if cache_key_mode == 'path':
        key = wav_path
    elif cache_key_mode == 'realpath':
        key = str(Path(wav_path).resolve())
    else:
        raise ValueError(f"Unsupported cache_key_mode: {cache_key_mode}")
    return blake2b(key.encode('utf-8'), digest_size=16).hexdigest()


def cache_path_for(wav_path: str, split: str, cache_dir: Path, cache_key_mode: str, cache_layout: str) -> Path:
    h = cache_key_for(wav_path, cache_key_mode)
    subdir = split if cache_layout == 'split' else 'shared'
    return cache_dir.joinpath(subdir, f"{h}.npy")


def precompute_cache(files: list[str], split: str, cache_dir: Path,
                     seq_len: int, n_mfcc: int, n_mel: int, n_chroma: int,
                     sr: int, hop_length: int, n_fft: int, num_features: int,
                     cache_key_mode: str = 'path', cache_layout: str = 'split') -> list[str]:
    out = []
    subdir = split if cache_layout == 'split' else 'shared'
    cache_dir.joinpath(subdir).mkdir(parents=True, exist_ok=True)
    for p in files:
        out_p = cache_path_for(p, split, cache_dir, cache_key_mode=cache_key_mode, cache_layout=cache_layout)
        if not out_p.exists():
...
def main():
    ...
    parser.add_argument('--cache_key_mode', choices=['path', 'realpath'], default='path', help="Cache key uses symlink path ('path') or resolved real path ('realpath')")
    parser.add_argument('--cache_layout', choices=['split', 'shared'], default='split', help="Store caches under split subfolders, or a shared folder usable across splits")
    ...
    parser.add_argument('--wandb_project', type=str, default='guitar-technique-cnn-lstm', help='W&B project name')
    parser.add_argument('--wandb_entity', type=str, default=None, help='W&B entity (team/user)')
    parser.add_argument('--wandb_run_name', type=str, default=None, help='W&B run name (defaults to experiment_name)')
    parser.add_argument('--wandb_tags', type=str, default=None, help='Comma-separated tags for W&B run')
    ...
    if args.wandb:
        if not WANDB_AVAILABLE:
            print("[WARN] wandb requested but not installed. Run: pip install wandb")
        else:
            wandb_config = {
            ...
            wandb_run = wandb.init(
                ...
                name=args.wandb_run_name or args.experiment_name or run_stamp,
                config=wandb_config,
                tags=wandb_tags,
                dir=str(log_dir),
            )
            print(f"[INFO] Wandb run initialized: {wandb_run.url}")
    ...
    if args.consolidated_dir is not None:
        ...
        if args.use_feature_cache:
            ...
            if args.rebuild_cache:
                ...
                if args.cache_layout == 'shared':
                    split_dirs = (cache_dir / 'shared',)
                else:
                    split_dirs = tuple(cache_dir / s for s in ("train", "val", "test"))
                for split_dir in split_dirs:
                    if split_dir.exists():
                        shutil.rmtree(split_dir)
            X_tr_cached = precompute_cache(
                X_tr_files, 'train', cache_dir,
                args.seq_len, args.n_mfcc, args.n_mel, args.n_chroma,
                args.sr, args.hop_length, args.n_fft, num_features,
                cache_key_mode=args.cache_key_mode, cache_layout=args.cache_layout,
            )
            X_va_cached = precompute_cache(
                X_va_files, 'val', cache_dir,
                args.seq_len, args.n_mfcc, args.n_mel, args.n_chroma,
                args.sr, args.hop_length, args.n_fft, num_features,
                cache_key_mode=args.cache_key_mode, cache_layout=args.cache_layout,
            )
            X_te_cached = precompute_cache(
                X_test_files, 'test', cache_dir,
                args.seq_len, args.n_mfcc, args.n_mel, args.n_chroma,
                args.sr, args.hop_length, args.n_fft, num_features,
                cache_key_mode=args.cache_key_mode, cache_layout=args.cache_layout,
            ) if len(X_test_files) else []
