...
def run_technique_model_on_chunks(chunk_paths: List[str], onsets, durations):
    PYTHON_EXECUTABLE = "./venv/.venv/bin/python"

    BASE_DIR="./data/music_ai_pipeline/"
    INPUT_DIR="./data/music_ai_pipeline/audio_slices/"
    
    MODEL_DIR="./data/music_ai_pipeline/expTechInfer_12-14-2025/models_cnn_lstm/setupB-eg_ipt-plus4/run-20251212-231215"
    MODEL_FILE="./data/music_ai_pipeline/expTechInfer_12-14-2025/models_cnn_lstm/setupB-eg_ipt-plus4/run-20251212-231215/cnn_lstm_best.h5"

    INFERENCE_FILE = "./data/music_ai_pipeline/expTechInfer_12-14-2025/scripts/infer_cnn_lstm.py"
...
def cached_technique_result(audio_path, midi_dict_techniques, force_rerun=False):
    """
    Cached expressive technique predictions to avoid recomputing.
    
    Args:
        audio_path: Path to audio file
        midi_dict_techniques: List of MIDI note dicts for model
    ...
    cache_dir = Path("cache/techniques")
    ...
    if not force_rerun and cache_file.exists():
        print(f"✓ Loading techniques from cache: {cache_file}")
        try:
    ...
    print(f"⟳ Running technique model (no cache or force rerun)...")
    
    # Extract audio chunks and run model
    chunk_paths, midi_onsets, midi_durations = audio_midi_to_chunks(audio_path, midi_dict_techniques)
    exp_onset_dur_tuples = run_technique_model_on_chunks(chunk_paths, midi_onsets, midi_durations)
