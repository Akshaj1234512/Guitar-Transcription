...
def normalize_audio(audio):
    ...
    if np.max(np.abs(audio)) > 1.0:
        # If audio is outside [-1, 1], normalize it
...
class RegressionPostProcessor(object):
    ...
    def output_dict_to_note_pedal_arrays(self, output_dict):
        ...
        (onset_output, onset_shift_output) = \
            self.get_binarized_output_from_regression(
                ...
                threshold=self.onset_threshold, neighbour=1)
        ...
        (offset_output, offset_shift_output) = \
            self.get_binarized_output_from_regression(
                ...
                threshold=self.offset_threshold, neighbour=1)
        ...
        if 'reg_pedal_offset_output' in output_dict.keys():
            ...
            (pedal_offset_output, pedal_offset_shift_output) = \
                self.get_binarized_output_from_regression(
                    ...
                    threshold=self.pedal_offset_threshold, neighbour=2)
    ...
    def output_dict_to_detected_notes(self, output_dict):
        ...
        MIN_NOTE_DURATION = 0.05  # 50ms - guitar notes are rarely shorter
    
        filtered_notes = []
        for i in range(len(est_on_off_note_vels)):
            onset = est_on_off_note_vels[i, 0]
            offset = est_on_off_note_vels[i, 1]
            duration = offset - onset
            
            if duration >= MIN_NOTE_DURATION:
                filtered_notes.append(est_on_off_note_vels[i])
        
        if len(filtered_notes) == 0:
            return np.array([])
        
        return np.array(filtered_notes)
