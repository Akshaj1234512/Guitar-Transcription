### Methods  to preprocess jams to add predictions

import jams
import pretty_midi
import tab_generation_utils.choose_best_position as position
...
# change added 01/02: to ensure does not take the same prediction twice
def midi_to_jams_with_tablature_from_sf_assignment(midi_path, string_fret_time_tuples, bpm = 120, tuning=STANDARD_TUNING,time_tolerance=0.2, capo = 0):
    '''function mostly borrowed from technique tabs, with the addition of aligning string/fret predictions with MIDI'''
    # load midi
    ...
    for idx, (string, fret, _, onset_sec) in enumerate(string_fret_time_tuples):
        sf_predictions.append((onset_sec, int(string), int(fret), idx))
    ...
    # print("\n=== TIMING DEBUG ===")
    # print("First 10 MIDI notes vs closest String/fret predictions:")
    # found_offsets = False

    # for i, n in enumerate(guitar_notes):
    #     # Find the single closest prediction in time
    #     closest_pred = min(sf_predictions, key=lambda x: abs(x[0] - n.start))
    ...
    #         print(f"    MIDI: {n.start:.4f}s | String/Fret: {closest_time:.4f}s")
    ...
    for n in guitar_notes:
        ...
        for onset_sec, string, fret, idx in sf_predictions:
